...
// --- Order is the whole point (spec §10) ------------------------------------
...
// Everything else (index.html, styles.css, logo.svg) keeps the default
// revalidate-always behaviour so a new deploy is picked up immediately.
app.use(
  "/assets",
  express.static(path.join(distPath, "assets"), { immutable: true, maxAge: "1y" })
);
app.use(express.static(distPath));
// Pathless app.use, not app.get("*") — the one SPA-fallback form that works
// identically on Express 4 and 5 (spec §10).
app.use((_req, res) => {
